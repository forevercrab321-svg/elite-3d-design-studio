...
description: |
  ...
  .io 病毒传播与短视频达人营销共 6 个流派，提炼 4 条共识、1 条单流派强假设、4 处分歧和一套网页多人游戏的发行打法。
...
5. **假设级流派要标出来**：引用 .io 或短视频流派的内容时，一律以「未核实假设：」开头，并说明怎么用我们自己的数据验证（写出埋点名）。研究里标【推断】的渠道顺序，回答时也保留「推断」二字。

## 框架概览：4 条跨流派共识 + 1 条单流派强假设
...
- （.io 传统的「不注册、几秒进入」与此方向一致，但属未核实假设，不计入流派支持数。）
...
### 单流派结论（GameDiscoverCo，不计入共识）：多人、低门槛、能截出高光，是小团队爆款的共同条件
> 只有 GameDiscoverCo 一个核实过的流派支持；案例部分多为未核实的记忆。当作强假设使用。
...
- Carless：**首周销量**大约是发售前 wishlist 的 0.1–0.2 倍，做销量和收入预期时按这个区间估，不要按 0.3 倍以上算。
...
| **网页门户** | 加载即转化；平台按测试数据放量；Poki：平台带来的流量 50/50，开发者自带的流量 100% 归开发者，Web 独占（一份调研为 5 年、BD 的平台档案为 7 年，以合同为准）★；CrazyGames：不要求独占，内购只对受邀游戏开放、需走其 Xsolla ★ | Poki、CrazyGames、itch.io、Newgrounds | 数据全来自平台自己；Poki 首包 ≤5MB 对 3D 游戏很难 ★；门户测试偏好「前 3 分钟好玩」，要等朋友的多人游戏会被低估 |
| **.io 病毒传播**（假设级，未核实） | 即时进入、一个动词、可逆的大鱼吃小鱼制造情绪峰值；靠 YouTuber 传播而非买量 | 自有域名、门户 | 大部分依据未核实；2015–16 年的空窗期不可复制；外挂和服务器成本 |
| **短视频与达人**（假设级，未核实） | 前 1–3 秒定生死；传播单位是 5–15 秒的瞬间；给创作者「好内容的原料」而不是买硬广 | TikTok、抖音、Shorts、B站、小红书 | 本次调研基本未核实；抖音、小红书禁止放网址（见 `docs/business/sales/tiktok-7day-plan.md`）；国内商业化涉及版号与资质 |

## 4 处流派分歧（原样保留）

1. **攒量还是等爆发**：Zukowski 主张发售前积累 wishlist；Peak 的 29 倍转化说明爆款的销量主要来自发售后的病毒传播。两者可以共存（先有人带一波），但对「预算花在发售前还是发售后」答案不同。
2. **独占换曝光，还是守住自有流量**：Poki 用多年 Web 独占（5 或 7 年，两份调研不一致）换首页位；自有域名和 .io 路线要把用户关系留在自己手里。
...
> 缺口：「头部主播一击 vs 中小创作者铺量」写不出有来源的双方立场，放在诚实边界。
...
3. Poki：等拿到 CrazyGames 的数据后再评估。首包 5MB 的限制和多年 Web 独占（5 或 7 年，以合同为准）都要认真算清楚（独占是否覆盖自有网站需书面确认）。
...
- **缺口**：「头部主播一击爆发 vs 中小创作者铺量」没有可引用的双方立场与数据。
