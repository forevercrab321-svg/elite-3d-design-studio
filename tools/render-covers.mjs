...
//   node tools/render-covers.mjs [background=marketing/press-kit/screenshots/shanghai-70s.png]
...
const bgPath = resolve(root, process.argv[2] ?? 'marketing/press-kit/screenshots/shanghai-70s.png');
...
const page = (w, h) => {
  ...
  * { box-sizing: border-box; }
  ...
  .bg { position: absolute; inset: 0; background: url(${bg}) center / cover no-repeat; filter: saturate(1.08) contrast(1.04); }
  .shade { position: absolute; inset: 0; background: ${
    wide
      ...
      : 'linear-gradient(0deg, rgba(22,24,26,.95) 0%, rgba(22,24,26,.78) 34%, rgba(22,24,26,0) 62%)'
  }; }
  ...
  .content { position: absolute; ${wide ? `left: ${Math.round(72 * s)}px; top: 50%; transform: translateY(-50%); width: 58%;` : `left: ${Math.round(40 * s)}px; right: ${Math.round(40 * s)}px; bottom: ${Math.round(56 * s)}px;`} }
